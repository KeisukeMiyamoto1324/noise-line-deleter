...
def create_sample_payload(
    ...
    text = str(row["text"])
    lines, labels = build_line_labels(text, list(row["lines_to_delete"]))
    ...
    line_payloads = [
        {
            "number": line_index + 1,
            "text": line,
            "actual_delete": labels[line_index] == 1,
            "predicted_delete": probabilities[line_index] >= 0.5,
            "noise_probability": probabilities[line_index],
        }
    ...
    return {
        ...
        "actual_delete_count": sum(labels),
        "predicted_delete_count": sum(1 for probability in probabilities if probability >= 0.5),
        "lines": line_payloads,
    }
...
def predict_line_probabilities(
    ...
    lines, _ = build_line_labels(text, lines_to_delete)
